...
class PositionSearchProblem(search.SearchProblem):
    ...
    def getGoalState(self):
        """Need goal state before hand for bi-directional search problem"""
        return self.goal
    ...
    def getCostOfActions(self, actions, backward=False):
        """
...
class CornersProblem(search.SearchProblem):
    ...
    def getGoalState(self):
        ...
        max_dist = float('-inf')
        for corner in self.corners:
            dist_from_start = manhattan_distance(self.startingPosition, corner)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hits_wall:
                if new_position in self.corners:
                    index = self.corners.index(new_position)
                    corners_visited_l = list(corners_visited)
                    corners_visited_l[index] = True
    ...
    def getSuccessorsBS(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hits_wall:
                if pacman_position in self.corners and new_position not in self.corners:
...
def cornersHeuristic(state, problem, info={}):
    ...
    if 'rev' in info and info['rev'] is True:
        in_corners = [corner for idx, corner in enumerate(corners) if corners_visited[idx]]
        manhattan_distances = []
        for corner in in_corners:
            for other_corner in in_corners:
                if corner != other_corner:
                    manhattan_distances.append(manhattan_distance(corner, other_corner))

        return min(manhattan_distances) if len(manhattan_distances) else 0

    else:
        if all(corners_visited):
            return 0

        corners_not_visited = [corner for idx, corner in enumerate(corners) if not corners_visited[idx]]
        manhattan_distances = []
        for corner in corners_not_visited:
            manhattan_distances.append(manhattan_distance(current_position, corner))
...
class FoodSearchProblem:
    ...
    def getGoalState(self):
        ...
        return tuple((farthest_food, foodGrid))
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                self.heuristicInfo['foodGrid'] = nextFood
...
def foodHeuristic(state, problem, info={}):
    """
    ...
    if 'rev' in info and info['rev']:
        if len(food_positions) == len(problem.heuristicInfo['foodGrid'].asList()):
            return 0
        food_positions = problem.heuristicInfo['foodGrid'].asList()
    ...
    maze_distances = []
    for food in food_positions:
        maze_distances.append(mazeDistance(position, food, gamestate))
